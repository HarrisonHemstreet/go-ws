...
func HandleRouteError(w http.ResponseWriter, err error) {
	...
	case "pq: duplicate key value violates unique constraint \"users_username_key\"":
		routeError(w, "A user with that username already exists.", "duplicate_username", http.StatusConflict)
	case "authentication failed: crypto/bcrypt: hashedPassword is not the hash of the given password":
		routeError(w, "Authentication Failed: Wrong password or username", "wrong_creds", http.StatusUnauthorized)
	default:
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
}

func routeError(w http.ResponseWriter, message, errorCode string, statusCode int) {
